...
5. **随机选择逻辑**
   - 随机选择从项目内的候选人中选择（使用项目的CandidateIDs）
   - 选择结果会记录到History表
   - 实现位置：`backend/internal/service/random.go`

6. **头像机制统一**
   - 头像信息同时存储在`Candidate.PhotoURL`和`CandidatePhoto.IsAvatar`中
   - 设置头像时，会同步更新两个字段
   - 删除照片时，如果删除的是当前头像：
     - 自动查找其他照片设为新头像
     - 如果没有其他照片，清空`Candidate.PhotoURL`
   - 确保两个字段始终保持一致
...
### 高优先级
- [ ] **批量删除照片API**：添加后端API支持批量删除多张照片，提高删除效率
- [ ] **JWT认证重构**：重构认证机制，使用JWT token替代当前的默认用户ID方式
- [ ] **设置后端API**：实现设置通过后端API保存，包括：
  - 随机头像开关
  - 默认配色
  - 默认表情
  - 自定义称呼
...
**最新修改** (2025-01-07):
- ✅ **随机选择逻辑**：从项目内的候选人中选择（使用项目的CandidateIDs）
- ✅ **统一头像机制**：
  - 删除照片时，如果删除的是当前头像，自动更新Candidate.PhotoURL
  - 如果删除头像后还有其他照片，自动将第一张设为新头像
  - 确保Candidate.PhotoURL和CandidatePhoto.IsAvatar保持一致
